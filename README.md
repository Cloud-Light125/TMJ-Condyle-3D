...
面向颞下颌关节 MRI 的单结构三维分割课设/实验项目：

    医生在 3D Slicer 中手工标注下颌髁突
      -> nnU-Net v2 3d_fullres 学习三维 mask
      -> grouped 5-fold 评价
      -> 五折 ensemble 自动分割新病例
      -> SlicerNNUnet + 3D Slicer 三维显示
...
代码、Slicer 标注模块、数据转换、QC、数据集构建、grouped 5-fold、训练/预测/评价/
模型导出和文档已经建立。当前环境与本地医学数据的实际结果要以
